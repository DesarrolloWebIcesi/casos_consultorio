...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
    ...
    // rjaramillo 2011-01-18
    // Documenté $t_def['type'] == CUSTOM_FIELD_TYPE_ENUM
...
//				$t_def['type'] == CUSTOM_FIELD_TYPE_ENUM ||
    ...
    // rjaramillo 2011-01-18
    // Agregué la condición if($t_def['require_report'])
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		$t_def = custom_field_get_definition( $t_id );
    // rjaramillo 2011-01-18
    // Agregué a la condición $t_def['require_report'] &&
		if(  $t_def['require_report'] && !custom_field_set_value( $t_id, $t_bug_id, gpc_get_custom_field( "custom_field_$t_id", $t_def['type'], '' ), false ) ) {
			error_parameters( lang_get_defaulted( custom_field_get_field( $t_id, 'name' ) ) );
			trigger_error( ERROR_CUSTOM_FIELD_INVALID_VALUE, ERROR );
		}
	}
...
<?php
    ...
    * rjaramillo - Richard Jaramillo Herrera
    * 2010-02-08
    * Agregado para que los informadores no puedan llegar al enlace view_all_bug_page.php
    */
    if ( current_user_get_access_level() > REPORTER)
    {
