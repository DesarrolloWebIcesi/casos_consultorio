...
// rjaramillo 2011-03-02
// Se agregó para obtener el nombre real del usuario que se trae de LDAP
      $nombre = ldap_get_field_from_username( $p_username, 'givenname' );
      $apellido = ldap_get_field_from_username( $p_username, 'sn' );
			$t_auto_create = true;
		} else {
			$t_auto_create = false;
		}
...
// rjaramillo 2011-03-02
// Se modificó para que almacene el nombre real del usuario que se trae de LDAP
      if ( LDAP == $t_login_method )
      {
        $t_cookie_string = user_create( $p_username, md5( $p_password ), '', null, false, true, $nombre.' '.$apellido );
      }
      else
      {
