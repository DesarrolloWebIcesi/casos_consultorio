...
<!--
rjaramillo 2011-01-28
Agregado para mostrar título de las columnas
-->
<tr bgcolor='#E6E6E6'>
  <td class="center"><b>ID</b></td>
  <td class="center"><b>Asunto</b></td>
  <td class="center" width="16%"><b>Fecha de solicitud</b></td>
</tr>
...
# -- Loop over bug rows and create $v_* variables --
    ...
    /**
     * rjaramillo - Richard Jaramillo Herrera
     * 2011-01-28
    ...
    * rjaramillo - Richard Jaramillo Herrera
    * 2010-02-25
    * Agregado para consultar el campo Fecha de envío que se desplegará más abajo
    */
    $t_date_submitted = date( config_get( 'normal_date_format' ), $t_bug->date_submitted );
    $t_date_submitted = str_replace($mes_ingles, $mes_espanol, $t_date_submitted);
...
<tr bgcolor="<?php echo $status_color?>">
	...
	<td class="center" valign="top" nowrap="nowrap">
		<span class="small">
		<?php
	...
	?>
        ...
        * rjaramillo - Richard Jaramillo Herrera
        * 2010-02-26
        * Agregado para consultar el campo Tipo de gestión y desplegarlo en la Vista en lugar del Tipo de servicio
        */
        $sql = "SELECT value FROM mantis_custom_field_string_table WHERE field_id='26' AND bug_id='".$t_bug->id."';";
        $resultado = db_fetch_array( db_query( $sql ) );
        if($resultado['value'] != "N/A" && $resultado['value'] != "")
        {
            echo "<span class=\"small\">".$resultado['value']."</span> - ";
        }
	...
	if( $t_bug->last_updated > strtotime( '-' . $t_filter[FILTER_PROPERTY_HIGHLIGHT_CHANGED] . ' hours' ) ) {
		echo '<b>Última actualización el ' . $t_last_updated . '</b>';
	} else {
		echo 'Última actualización el '.$t_last_updated;
	}
	?>
		</span>
	</td>
  <td><span class="small"><?php echo $t_date_submitted?></span></td>
</tr>
<?php
