...
+        *Fecha: 21/Mayo/2015
	...
	// rjaramillo 2011-01-27
    // Ocultando el campo Solución a los informadores
    // Le agregué a la condición: && current_user_get_access_level() > REPORTER
    $tpl_show_additional_information = !is_blank( $tpl_bug->additional_information ) && in_array( 'additional_info', $t_fields ) && current_user_get_access_level() > REPORTER;
	// rjaramillo 2011-01-27
    // Ocultando el campo Diagnóstico a los informadores
    // Le agregué a la condición: && current_user_get_access_level() > REPORTER
	$tpl_show_steps_to_reproduce = !is_blank( $tpl_bug->steps_to_reproduce ) && in_array( 'steps_to_reproduce', $t_fields ) && current_user_get_access_level() > REPORTER;
	$tpl_show_monitor_box = !$tpl_force_readonly;
	// rjaramillo 2011-01-27
    // Ocultando las relaciones a los informadores
    // Le agregué a la condición: && current_user_get_access_level() > REPORTER
    $tpl_show_relationships_box = !$tpl_force_readonly && current_user_get_access_level() > REPORTER;
